...
exports.forgotPassword = async (req, res) => {
  ...
  if (!validationUtil.isValidEmail(email)) {
    logger.debug("Forgot password - invalid email format", {
      correlationId,
      context: { email },
    });
    return res
...
exports.resetPassword = async (req, res) => {
  ...
  try {
      ...
      return res
        ...
        .json(
          responseUtil.unauthorized(
            req,
            `Password reset failed: ${tokenVerification.reason}`,
          ),
        );
    }
    ...
    return res
      .status(500)
      .json(responseUtil.serverError(req, "Failed to reset password"));
  }
};
...
exports.changePassword = async (req, res) => {
  ...
  if (!userId) {
    ...
    return res
      .status(401)
      .json(responseUtil.unauthorized(req, "User not authenticated"));
  }
  ...
  try {
    ...
    if (!user) {
      ...
      return res
        .status(401)
        .json(responseUtil.unauthorized(req, "User not found"));
    }
    ...
    if (!isPasswordValid) {
      ...
      return res
        .status(401)
        .json(responseUtil.unauthorized(req, "Current password is incorrect"));
    }
    ...
    return res
      .status(500)
      .json(responseUtil.serverError(req, "Failed to change password"));
  }
};
...
exports.unlockAccount = async (req, res) => {
  ...
  try {
    ...
    if (!user) {
      ...
      return res
        .status(404)
        .json(responseUtil.notFound(req, "User with this email not found"));
    }
    ...
    return res
      .status(500)
      .json(responseUtil.serverError(req, "Failed to unlock account"));
  }
};
...
exports.deleteAccount = async (req, res) => {
  ...
  // ===== VALIDATION =====
  if (!password) {
    logger.debug("Delete account - password missing", {
      correlationId,
      context: { userId },
    });
    return res
  ...
  try {
    ...
    if (!isPasswordValid) {
      ...
      return res
        .status(401)
        .json(responseUtil.unauthorized(req, "Password is incorrect"));
    }
    ...
    return res
      .status(500)
      .json(responseUtil.serverError(req, "Failed to delete account"));
  }
};
...
exports.verifyEmail = async (req, res) => {
  ...
  try {
    ...
    const tokenVerification =
      await tokenUtil.verifyEmailVerificationToken(token);
    if (!tokenVerification.is_valid) {
      ...
      return res
        ...
        .json(
          responseUtil.unauthorized(
            req,
            `Email verification failed: ${tokenVerification.reason}`,
          ),
        );
    }
    ...
    const user = await User.findOne({ where: { user_id: userId } });
    if (!user) {
      logger.warn("Verify email - user not found", {
        correlationId,
        context: { userId },
      });
      return res
    ...
    return res
      .status(500)
      .json(responseUtil.serverError(req, "Failed to verify email"));
  }
};
...
exports.resendVerificationEmail = async (req, res) => {
  ...
  try {
    ...
    const successResponse = {
      success: true,
      message:
        "If an account exists with this email and is not yet verified, a verification email will be sent shortly.",
      status_code: 200,
    };
    ...
    const { token } = await tokenUtil.createEmailVerificationToken(
      user.user_id,
      email,
    );
