...
exports.createUser = async (req, res) => {
  ...
  if (!validation.isValid) {
    ...
    return res.status(400).json(
      responseUtil.validationError(req, "Username and password required", {
  ...
  if (email && !validationUtil.isValidEmail(email)) {
    logger.warn("Create user email format invalid", {
      correlationId,
      context: { email },
    });
    return res.status(400).json(
  ...
  try {
    ...
    logger.info("User created successfully", {
      correlationId,
      context: { userId: newUser.user_id },
    });
    ...
    return res
      .status(500)
      .json(responseUtil.serverError(req, "Error creating user"));
  }
};
...
exports.getAllUsers = async (req, res) => {
  ...
  try {
    ...
    return res.json(
      responseUtil.success(req, `Retrieved ${result.total} users`, result),
    );
  } catch (error) {
    logger.error("Get all users failed", { correlationId, error });
    return res
      .status(500)
      .json(responseUtil.serverError(req, "Error retrieving users"));
  }
};
...
exports.getUserDetail = async (req, res) => {
  ...
  try {
    ...
    return res.json(
      responseUtil.success(req, "User retrieved successfully", user),
    );
  } catch (error) {
...
exports.updateUser = async (req, res) => {
  ...
  try {
    ...
    return res.json(
      responseUtil.success(req, "User updated successfully", updatedUser),
    );
  } catch (error) {
...
// Delete user - Hard delete user account
exports.deleteUser = async (req, res) => {
  ...
  try {
    ...
    logger.warn("Delete user failed", {
      correlationId,
      context: { userId: id },
    });
    return res.status(404).json(responseUtil.notFound(req, "User not found"));
...
exports.getUsersByBranch = async (req, res) => {
  ...
  const validation = validationUtil.validateRequiredFields({ branch_id }, [
    "branch_id",
  ]);
  if (!validation.isValid) {
    return res
      .status(400)
      .json(responseUtil.validationError(req, "branch_id is required"));
  }
  ...
  try {
    const branch = await db.Branch.findOne({
      ...
      include: [
        {
          model: db.User,
          as: "Users", // ✅ khớp alias trong index.js
    ...
    if (!branch) {
      ...
      return res
        .status(404)
        .json(responseUtil.notFound(req, "Branch not found"));
    }

    logger.info("Branch users retrieved", {
      correlationId,
      context: { branchId: branch_id, count: branch.Users.length },
    });

    return res.json(
      responseUtil.success(
        req,
        `Retrieved ${branch.Users.length} users from branch`,
        branch.Users,
      ),
    );
  } catch (error) {
    ...
    return res
      .status(500)
      .json(responseUtil.serverError(req, "Error retrieving branch users"));
  }
};
...
exports.assignUserToBranch = async (req, res) => {
  ...
  if (!validation.isValid) {
    return res
      .status(400)
      .json(
        responseUtil.validationError(req, "branch_id and user_id required"),
      );
  }

  try {
    const result = await userService.assignBranches(parseInt(user_id), [
      parseInt(branch_id),
    ]);

    logger.info("User assigned to branch", {
      correlationId,
      context: { userId: user_id, branchId: branch_id },
    });

    return res.json(
      responseUtil.success(req, "User assigned to branch successfully", result),
    );
  } catch (error) {
    logger.warn("Assign user to branch failed", {
      correlationId,
      context: { userId: user_id, branchId: branch_id },
      error: error.message,
    });
    return res.status(404).json(responseUtil.notFound(req, "User or branch"));
  }
};
...
exports.removeUserFromBranch = async (req, res) => {
  ...
  try {
    ...
    const branch = await db.Branch.findByPk(parseInt(branch_id));

    if (!user || !branch) {
      logger.warn("User or branch not found", { correlationId });
      return res
        .status(404)
        .json(responseUtil.notFound(req, "User or branch not found"));
    }

    // ✅ Raw query - tránh dùng magic method có thể không khớp alias
    await db.sequelize.query(
      `DELETE FROM userbranch WHERE user_id = :userId AND branch_id = :branchId`,
      {
        replacements: {
    ...
    logger.info("User removed from branch", {
      correlationId,
      context: { userId: user_id, branchId: branch_id },
    });

    return res.json(
      responseUtil.success(req, "User removed from branch successfully"),
    );
  } catch (error) {
    logger.error("Remove user from branch failed", { correlationId, error });
    return res
      .status(500)
      .json(responseUtil.serverError(req, "Error removing user from branch"));
  }
};
...
exports.assignRole = async (req, res) => {
  ...
  if (req.user?.role_id !== 1) {
    logger.warn("Unauthorized role assignment attempt", {
      correlationId,
      context: { adminId, targetUserId: user_id },
    });
    return res
      .status(403)
      .json(responseUtil.forbidden(req, "Only admin can assign roles"));
  }
  ...
  if (!validation.isValid) {
    return res
      .status(400)
      .json(responseUtil.validationError(req, "user_id and role_id required"));
  }

  if (![1, 2, 3, 4].includes(role_id)) {
    return res
      .status(400)
      .json(responseUtil.validationError(req, "Invalid role_id. Must be 1-4"));
  }
  ...
  try {
    ...
    return res.json(
      responseUtil.success(req, "Role assigned successfully", {
    ...
    return res
      .status(500)
      .json(responseUtil.serverError(req, "Error assigning role"));
  }
};
