...
exports.createItem = async (req, res) => {
  ...
  const { item_name, category_id, branch_id, item_description, price } =
    req.body;

  try {
    // Validate foreign keys first to avoid saving orphan images
    if (category_id) {
      const category = await db.MenuCategory.findOne({
    ...
    if (req.file) {
      ...
      try {
        const imagePath = imageUtil.saveItemImage(
          req.file.buffer,
          newItem.item_id,
        );
        ...
        logger.error("Failed to save item image after creating item", {
          correlationId,
          error: e,
        });
        return res
          .status(500)
          .json(responseUtil.serverError(req, "Lỗi khi lưu ảnh mục menu"));
      }
    }
    ...
    return res
      .status(201)
      .json(responseUtil.success(req, "Tạo mục menu thành công", created, 201));
  } catch (error) {
    if (error.name === "SequelizeUniqueConstraintError") {
      ...
      return res
        .status(409)
        .json(responseUtil.conflict(req, "RID mục menu đã tồn tại"));
    }
    ...
    return res
      .status(500)
      .json(responseUtil.serverError(req, "Lỗi server khi tạo mục menu"));
  }
};
...
exports.getAllItems = async (req, res) => {
  ...
  try {
    ...
    return res
      .status(500)
      .json(
        responseUtil.serverError(req, "Lỗi server khi lấy danh sách mục menu"),
      );
  }
};
...
exports.getItemDetail = async (req, res) => {
  ...
  try {
    ...
    return res.json(
      responseUtil.success(req, "Lấy thông tin mục menu thành công", item),
    );
  } catch (error) {
    ...
    return res
      .status(500)
      .json(
        responseUtil.serverError(req, "Lỗi server khi lấy thông tin mục menu"),
      );
  }
};
...
exports.updateItem = async (req, res) => {
  ...
  try {
    ...
    const [updatedRows] = await MenuItem.update(updatePayload, {
      where: { item_id: id, is_delete: false },
    });
    ...
    return res.json(
      responseUtil.success(req, "Cập nhật mục menu thành công", updatedItem),
    );
  } catch (error) {
    ...
    return res
      .status(500)
      .json(responseUtil.serverError(req, "Lỗi server khi cập nhật mục menu"));
  }
};
...
exports.deleteItem = async (req, res) => {
  ...
  try {
    ...
    return res
      .status(500)
      .json(responseUtil.serverError(req, "Lỗi server khi xóa mục menu"));
  }
};
...
exports.uploadItemImage = async (req, res) => {
  ...
  try {
    ...
    if (updatedRows === 0) {
      ...
      return res
        .status(500)
        .json(responseUtil.serverError(req, "Không thể lưu thông tin ảnh"));
    }
    ...
    return res
      .status(500)
      .json(
        responseUtil.serverError(req, "Lỗi server khi tải lên ảnh mục menu"),
      );
  }
};
